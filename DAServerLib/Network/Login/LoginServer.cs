...
namespace Darkages.Network.Login
{
    public class LoginServer : NetworkServer<LoginClient>
    {
        public LoginServer(int capacity)
            : base(capacity)
        {
            MServerTable = MServerTable.FromFile(ServerContext.SERVERTABLE);
            Notification = Notification.FromFile(ServerContext.NEWS);
        }
        ...
        public void LoginAsAisling(LoginClient client, Aisling _aisling)
        {
            if (_aisling != null)
            {
                ...
                if (PlayerIsOneOf(_aisling.Username, ServerContext.Config.GameMasters))
                {
        ...
        private bool PlayerIsOneOf(string username, string[] gameMasters)
        {
            if (gameMasters == null || gameMasters.Length == 0)
                return false;

            if (string.IsNullOrEmpty(username))
                return false;

            foreach (var o in gameMasters)
            {
                if (o.Equals(username, StringComparison.OrdinalIgnoreCase))
                    return true;
            }
