...
namespace Darkages
{
    ...
    public class ServerContext : ObjectManager
    {
        ...
        static ServerContext()
        {
            ...
            StoragePath = Path.Combine(GetFolderPath(SpecialFolder.MyDocuments), "LORULE_DATA");
        }
        ...
        public static IPAddress IPADDR { get; internal set; }
        ...
        public static string SERVERTABLE { get; set; }
        ...
        public virtual void Start(params object[] args)
        {
            ...
            if (args.Length == 0)
            {
                throw new Exception("Missing: Server Main Args.");
            }

            try
            {
                IPADDR        = IPAddress.Parse(File.ReadAllText(args[0] as string));
                DAT           = args[1] as string;
                SERVERTABLE   = args[2] as string;
                NEWS          = args[3] as string;
            }
            catch
            {
                throw new Exception("Critical Error: Main Args Missing.");
            }
